...
const INTERVALS = ["monthly","quarterly","biannual","annual","custom"];

export function RecurringSchedulesPage() {
  const schedules = (useQuery(api.recurringSchedules.list) ?? []) as any[];
  ...
  async function handleCreate() {
    ...
    try {
      await create({ customerId: form.customerId as Id<"customers">, employeeId: form.employeeId ? form.employeeId as Id<"employees"> : undefined, serviceType: form.serviceType, intervalType: form.intervalType as any, customDays: form.customDays ? Number(form.customDays) : undefined, startDate: form.startDate, revenue: form.revenue, notes: form.notes || undefined });
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div><h1 className="text-2xl font-bold flex items-center gap-2"><RefreshCw className="size-6" /> Recurring Schedules</h1><p className="text-sm text-gray-500">{schedules.filter((s: any) => s.status === "active").length} active</p></div>
