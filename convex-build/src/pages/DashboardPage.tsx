...
export function DashboardPage() {
  ...
  const leaks = useQuery(api.dashboard.profitLeaks) ?? [];
  const customers = useQuery(api.customers.list) ?? [];
  const createJob = useMutation(api.jobs.create);

  const loading = summary === undefined;

  const completedJobs = (jobs as any[]).filter((j: any) => j.status === "completed").length;
  const activeJobs    = (jobs as any[]).filter((j: any) => j.status === "in-progress" || j.status === "in_progress").length;
  const pendingJobs   = (jobs as any[]).filter((j: any) => j.status === "pending").length;
  const issueJobs     = (jobs as any[]).filter((j: any) => j.status === "return" || j.status === "will_return" || j.status === "reschedule").length;
  const totalJobs     = (jobs as any[]).length || 1;

  const pendingInvoices = (invoices as any[]).filter((inv: any) => inv.status === "draft" || inv.status === "pending");
  const overdueInvoices = (invoices as any[]).filter((inv: any) => inv.status === "overdue");
  const pendingInvoiceTotal = pendingInvoices.reduce((s: number, inv: any) => s + (inv.totalAmount ?? 0), 0);

  const activeEmployees = (employees as any[]).filter((e: any) => e.isActive);
  ...
  async function handleEmergencyDispatch() {
    ...
    try {
      const custId = (customers as any[]).find((c: any) => c._id === emergencyForm.customerId)?._id;
    ...
    <div className="space-y-6">
      ...
      >
        ...
        <div className="grid grid-cols-3 gap-3">
          ...
          <div className="rounded-lg border p-3 text-center">
            ...
            <div className="text-xl font-extrabold">{loading ? "—" : (invoices as any[]).length}</div>
            <div className="text-[10px] text-gray-400">${loading ? "—" : (invoices as any[]).reduce((s: number, inv: any) => s + (inv.totalAmount ?? 0), 0).toLocaleString()} billed</div>
          </div>
        </div>
      </div>

      {/* Profit Leak Alerts */}
      {(leaks as any[]).length > 0 && (
        <div className="bg-red-50 border border-red-200 rounded-xl p-4 sm:p-5">
          ...
          <div className="space-y-2">
            {(leaks as any[]).map((leak: any, i: number) => (
              <div
                key={i}
      ...
      <div className="bg-white rounded-xl border overflow-hidden">
        ...
        <div className="divide-y">
          ...
          {[...(jobs as any[])].sort((a, b) => (b._creationTime ?? 0) - (a._creationTime ?? 0)).slice(0, 6).map((job: any) => (
