...
import type { ConvexTimeOff, ConvexEmployee } from "@/lib/convex-types";
...
export function TimeOffPage() {
  const { user } = useAuth();
  const requests = (useQuery(api.timeoff.list) ?? []) as ConvexTimeOff[];
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div><h1 className="text-2xl font-bold flex items-center gap-2"><Clock className="size-6" /> Time Off</h1><p className="text-sm text-gray-500">{requests.filter(r => r.status === "pending").length} pending requests</p></div>
