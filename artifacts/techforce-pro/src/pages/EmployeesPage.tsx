...
function EditEmployeeDialog({
  ...
  const [form, setForm] = useState({
    ...
    hourlyRate: String(emp.hourlyRate != null ? Number(emp.hourlyRate).toFixed(2) : (emp.salary / 2080).toFixed(2)),
    hoursPerDay: String(emp.hoursPerDay ?? 8),
    homeZip: emp.homeZip ?? "",
  ...
  async function handleSave() {
    setSaving(true);
    try {
      const hr  = Number(form.hourlyRate) || (emp.hourlyRate != null ? Number(emp.hourlyRate) : emp.salary / 2080);
      ...
      const updated = await updateEmployee(emp.id, {
        ...
        hourlyRate: hr,
        hoursPerDay: hpd,
        salary: Math.round(hr * hpd * 260),
        billableRate: Math.round(hr * hpd),
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <Input type="number" step="0.01" value={form.hourlyRate} onChange={e => setField("hourlyRate", e.target.value)} />
            </div>
            <div>
              <Label className="text-xs">Hours / Day</Label>
              <Input type="number" step="0.5" value={form.hoursPerDay} onChange={e => setField("hoursPerDay", e.target.value)} />
            </div>
          </div>
          <p className="text-[10px] text-muted-foreground -mt-1">
            Annual est. ${isNaN(Number(form.hourlyRate)) ? "—" : (Number(form.hourlyRate) * (Number(form.hoursPerDay) || 8) * 260).toLocaleString()} · Daily wage ${isNaN(Number(form.hourlyRate)) ? "—" : (Number(form.hourlyRate) * (Number(form.hoursPerDay) || 8)).toFixed(0)} · Bill rate = actual job revenue per day
          </p>
...
function AddEmployeeDialog({ open, onClose, onAdd }: { open: boolean; onClose: () => void; onAdd: (emp: ApiEmployee) => void }) {
  const [form, setForm] = useState({
    name: "", role: "extinguisher_tech", hourlyRate: "", hoursPerDay: "8", certs: "", homeZip: "",
  });
  const [saving, setSaving] = useState(false);
  ...
  async function handleAdd() {
    if (!form.name.trim() || !form.hourlyRate) {
    ...
    try {
      ...
      onAdd(emp);
      onClose();
      setForm({ name: "", role: "extinguisher_tech", hourlyRate: "", hoursPerDay: "8", certs: "", homeZip: "" });
    ...
    <Dialog open={open} onOpenChange={o => !o && onClose()}>
      <DialogContent>
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div><Label className="text-xs">Hourly Rate ($/hr) *</Label><Input type="number" step="0.01" placeholder="26.50" value={form.hourlyRate} onChange={e => setField("hourlyRate", e.target.value)} /></div>
            <div><Label className="text-xs">Hours / Day</Label><Input type="number" step="0.5" placeholder="8" value={form.hoursPerDay} onChange={e => setField("hoursPerDay", e.target.value)} /></div>
          </div>
          {form.hourlyRate && !isNaN(Number(form.hourlyRate)) && (
            <p className="text-[10px] text-muted-foreground -mt-1">
              Daily wage ≈ ${(Number(form.hourlyRate) * (Number(form.hoursPerDay) || 8)).toFixed(0)} · Annual est. ${(Number(form.hourlyRate) * (Number(form.hoursPerDay) || 8) * 260).toLocaleString()} · Bill rate is computed from actual job revenue
            </p>
          )}
            ...
            <Card>
              <CardContent className="p-4">
                ...
                <div className="text-xl font-extrabold">
                  {showHourly
                    ? `$${active.length > 0 ? (active.reduce((s, e) => s + (e.hourlyRate != null ? Number(e.hourlyRate) : e.salary / 2080), 0) / active.length).toFixed(2) : "—"}/hr`
                    ...
                    <div className="grid grid-cols-3 gap-2 text-center">
                      <div className="bg-emerald-50 dark:bg-emerald-900/20 rounded-lg p-2.5">
                        ...
                        <div className="text-sm font-extrabold text-emerald-700 dark:text-emerald-400">
                          ${emp.hourlyRate != null ? Number(emp.hourlyRate).toFixed(2) : toHourly(emp.salary)}/hr
                        </div>
                      </div>
