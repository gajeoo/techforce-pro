...
import {
  ...
  FileText, Receipt, PlusCircle, ClipboardList, Building2, Activity,
} from "lucide-react";
import {
...
export function DashboardPage() {
  ...
  // Monthly revenue from invoices for sparkline
  const monthlyRevenue = useMemo(() => {
    const map = new Map<string, number>();
    invoices.forEach(inv => {
      const d   = new Date(inv.generatedAt);
      const key = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}`;
      map.set(key, (map.get(key) ?? 0) + inv.totalAmount);
    });
  ...
  const completedPct = (completedJobs / totalJobs) * 100;
  const activePct = (activeJobs / totalJobs) * 100;
  const pendingPct = (pendingJobs / totalJobs) * 100;
  const issuePct = (issueJobs / totalJobs) * 100;
  const activeStart = completedPct;
  const pendingStart = completedPct + activePct;
  const issueStart = completedPct + activePct + pendingPct;
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        <div className="flex items-center gap-4 self-start sm:self-auto">
          ...
          <div className="hidden sm:flex flex-col items-end text-right">
            <span className="text-sm font-mono font-bold tabular-nums">{liveTime}</span>
          ...
          <CardContent className="p-4 sm:p-5">
            <div className="flex items-center gap-2 mb-4">
              ...
              <span className="text-xs text-muted-foreground">
                · Last {monthlyRevenue.length} month{monthlyRevenue.length !== 1 ? "s" : ""}
              </span>
              <span className="ml-auto text-xs font-semibold text-emerald-600">
                ${monthlyRevenue.reduce((s, m) => s + m.revenue, 0).toLocaleString()} total
              </span>
            </div>
      ...
      <Card className="cursor-pointer hover:shadow-sm transition-shadow" onClick={() => navigate("/jobs")}>
        <CardContent className="p-4 sm:p-5">
          ...
          <div className="h-3 rounded-full overflow-hidden mb-3 bg-muted" role="img" aria-label="Jobs by status">
            <svg viewBox="0 0 100 12" className="w-full h-full" preserveAspectRatio="none">
              {completedPct > 0 && <rect x={0} y={0} width={completedPct} height={12} fill="#22c55e" />}
              {activePct > 0 && <rect x={activeStart} y={0} width={activePct} height={12} fill="#3b82f6" />}
              {pendingPct > 0 && <rect x={pendingStart} y={0} width={pendingPct} height={12} fill="#9ca3af" />}
              {issuePct > 0 && <rect x={issueStart} y={0} width={issuePct} height={12} fill="#f59e0b" />}
            </svg>
          </div>
